...
export const checkIfNoChanges = async () => {
  ...
  console.log(
    `⏳ Time since last file change: ${timeSinceLastChange / 1000} sec`,
  );
...
export const sendNoChangesNotification = async () => {
  ...
  const lastNoChangesTimestamp = lastNoChangesTime
    ? Number(lastNoChangesTime)
    : 0;
  const currentTime = Date.now();
  const timeSinceLastNoChanges = currentTime - lastNoChangesTimestamp;

  console.log(
    `🕒 Time since last 'No Changes' notification: ${timeSinceLastNoChanges / 1000} sec`,
  );
  ...
  if (noChangesLock && lockTTL > 0) {
    console.warn(
      `⚠️ Skipping duplicate 'No Latest Changes' notification due to lock. Expires in ${lockTTL / 1000} sec`,
    );
    return;
  }
